...
class DentalDocumentation(BaseModel):
    ...
    def calculate_confidence_and_review(self):
        """Calculate overall confidence and review requirement after all fields are set"""
        transcription = self.transcription
        billing_codes = self.billing_codes or []
        ...
        if not transcription:
            ...
            if base_confidence >= 0.9:
                ...
                self.overall_confidence = ConfidenceLevel.UNSURE

        # Determine requires_review
        if self.overall_confidence in [ConfidenceLevel.LOW, ConfidenceLevel.UNSURE]:
            self.requires_review = True
        elif self.procedures_performed and not billing_codes:
            self.requires_review = True
        else:
