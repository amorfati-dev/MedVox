...
type Props = {
  ...
  error: string | null;
  onChoose: (id: number | null) => void; // null = ohne Behandler diktieren
  onClose: () => void;
};

export function DentistPicker({ active, current, error, onChoose, onClose }: Props) {
  useEffect(() => {
    ...
    <div className="picker-backdrop">
      <section className="picker dentist-picker" role="dialog" aria-modal="true" aria-labelledby="dentist-title">
        <header className="picker-head">
          <h2 id="dentist-title">Wer diktiert?</h2>
          <button type="button" className="btn btn-icon" aria-label="Schließen" onClick={onClose}>
            <Icon name="x" />
          </button>
        </header>
        <p className="muted">Gilt auf diesem iPad ab dem nächsten Diktat, bis jemand wechselt.</p>
