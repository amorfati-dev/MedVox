...
export type Transfer = {
  ...
  error: string | null;
  send: (dictationId: string | null) => Promise<void>; // gespeichertes Diktat, das der Abruf schließt
};
...
export function useTransfer(
  ...
  const send = async (dictationId: string | null) => {
    const sent = version.current;
    setBusy(true);
    setError(null);
    try {
      const created = await api.createTransfer(transcript, codes, details ?? undefined, dictationId);
