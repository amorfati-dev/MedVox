...
| `GET /catalog?patient_type=kasse\|privat` | ja | – | `{"version", "patient_type", "entries": [{code, system, title, area, points, kind, evident, family, per_tooth, roots}]}` – Positionen aus `catalog_v1.json`, die für den Patiententyp vorgeschlagen werden dürfen (wie der Extraktor: Kasse = BEMA inkl. Analogpositionen plus Zuzahlungs-Liste, Privat = GOZ/GOÄ, Zuschlag 0500–0530 nur Privat); `family` = Ziffern nach Flächenzahl 1–4 (13a–13d) oder leer; `per_tooth` = `max_per.unit` „zahn“ (Zähne am iPad antippbar), `roots` = Paar nach Wurzelzahl (`4050`/`4055`, `AITa`/`AITb`) oder leer. Für das Katalog-Blatt am iPad, keine freie Ziffern-Eingabe |
