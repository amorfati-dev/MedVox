...
Logs enthalten weder Audio noch Transkripttext.
...
| `WHISPER_PROMPT_FILE` | `<repo>/infra/whisper/prompt.txt` | Diktat-Prompt für whisper; fehlt die Datei, gilt `FALLBACK_PROMPT` aus `medvox/settings.py` |
...
Transfer-Abrufe ≤ 10 pro Minute und IP.
...
| `POST /login` | nein | JSON `{"password": "…"}` | 204 + HttpOnly-Cookie `medvox_session` (SameSite=Strict); 401 falsches Passwort, 503 kein Hash konfiguriert |
...
`auth.py` (PBKDF2, Sitzungen), `transfer.py` (Kurzcodes, Rate-Limit), `db.py` (SQLite),
