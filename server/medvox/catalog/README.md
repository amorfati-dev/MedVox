...
(`Catalog.stepper`): je Kanal (Regel „je Kanal“), oder `sitzung` mit `count` über 1 nur bei `bestaetigt` (nie aus einem Vorschlag) – wer eine Grenze über einen längeren Zeitraum bestätigt (IP4, 4000, 4005), gibt ihr damit − / + bis `count` je Zeile. Bereiche aus der FDI-Nummer: Frontzahnbereich = 13–23 bzw. 33–43,
