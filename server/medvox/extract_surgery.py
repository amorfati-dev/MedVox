...
Analogposition, BEMA hat über Ost2 nichts; beim Privatpatienten GOZ 3045 (2026-09-26), nie Ä2650: § 6 Abs. 2 GOZ
öffnet nur GOÄ-Leistungen, die nicht in der GOZ stehen. Sie ersetzt Ost1/Ost2 am selben Zahn, nie beide in den
Vorschlägen, und gilt überall als Osteotomie: Ä1/Zst-Optionen am Weisheitszahn, Pla0 und Nbl2 wie neben Ost1/Ost2;
beim Privatpatienten bestimmt 3045 (767 Punkte) wie jede GOZ-Chirurgie den Zuschlag 0500–0530.
"""
...
SEVERE_OSTEOTOMY = {"kasse": ("GOÄ", "Ä2650"), "privat": ("GOZ", "3045")}
SEVERE = frozenset(SEVERE_OSTEOTOMY.values())
OST1_OST2 = frozenset({("BEMA", "47a"), ("BEMA", "48"), ("GOZ", "3030"), ("GOZ", "3040")})
OSTEOTOMY = OST1_OST2 | SEVERE
...
def severe_entry(catalog: Catalog, tagged: list[Tagged], patient: str) -> list[Tagged]:
    """„schwere Ost“ im System des Patienten: Kasse Ä2650, Privat 3045 – auch eine diktierte Ziffer des anderen."""
    target = catalog.get(*SEVERE_OSTEOTOMY[patient])
    result = []
    for t in tagged:
        entry = t.hit.entry
        if entry.key not in SEVERE or entry.key == target.key:
            result.append(t)
            continue
        result.append(replace(t, hit=replace(t.hit, entry=target, via=entry if t.hit.code_word else None)))
    return result


def severe_osteotomy(primaries: list[Draft]) -> list[Draft]:
    """Ä2650/3045 an einem Zahn übernimmt die Fundstellen einer Osteotomie (47a/48 bzw. 3030/3040) desselben Zahns."""
    severe = {d.fdi: d for d in primaries if d.entry.key in SEVERE and d.fdi is not None}
    kept = []
    for d in primaries:
        ...
        if host is None:
            kept.append(d)
            continue
        absorbed = [replace(t, hit=replace(t.hit, via=None)) for t in d.hits]  # kein Paar Ost2 ↔ Ä2650/3045 anzeigen
...
def split_bleeding(ctx: TextContext, primaries: list[Draft], notes: list[str]) -> tuple[list[Draft], list[Draft]]:
    ...
    surgery = any(d.entry.code in REMOVAL.get(d.entry.system, {}).values() or d.entry.key in SEVERE
                  for d in primaries)
    kept, offers = [], []
    for d in primaries:
