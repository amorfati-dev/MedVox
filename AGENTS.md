...
- Diktate je Patient (`server/medvox/patients.py`, App `/patienten`): Patient ist nur die Evident-Nummer; ein Diktat bleibt bis „übertragen“, höchstens 24 h (Vorgabe des Behandlers, nicht verlängerbar) und wird wie Kurzcodes bei Schreiben, Lesen, Start und periodisch aufgeräumt (`db.purge_expired`). Kurzcode/`/transfer` bleibt daneben die sofortige Übergabe.
